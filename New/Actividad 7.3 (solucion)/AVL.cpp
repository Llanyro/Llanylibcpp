...
Nodo* AVL::buscarHueco(Nodo *raizSubarbol, int elementoAInsertar) {
	assert(raizSubarbol != NULL);

	if (elementoAInsertar <= raizSubarbol->getContenido())
		if (raizSubarbol->getHijoIzquierdo() != NULL) return (buscarHueco(raizSubarbol->getHijoIzquierdo(), elementoAInsertar));
		else return (raizSubarbol);
	else
		if (raizSubarbol->getHijoDerecho() != NULL) return (buscarHueco(raizSubarbol->getHijoDerecho(), elementoAInsertar));
		else return (raizSubarbol);
...
Nodo* AVL::rotarSimpleIzquierda(Nodo *raizSubarbol) {
	...
	Nodo *x = raizSubarbol;
	Nodo *y = hijoDerecho;
	Nodo *B = hijoDerecho->getHijoIzquierdo();
