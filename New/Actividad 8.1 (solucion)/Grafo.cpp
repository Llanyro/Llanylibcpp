...
string Grafo::recorrerEnProfundidadRecursivo(int indiceNodoInicial, bool *vistos) {
	...
	for (int i=0; i<n; i++) {
		...
		if (vistos[i] == false) {
			...
			// Obtenemos el valor de esa celda de la matriz de adyacencia
			int existeArco = matriz[indiceNodoInicial].getValor(i);

			// Si es 1, es porque hay arco. En ese caso, hacemos la llamada recursiva y añadimos el resultado
			if (existeArco > 0) resultado.append(recorrerEnProfundidadRecursivo(i, vistos));
		}
	}
	return (resultado);
}
...
char* Grafo::recorrerEnProfundidad(char nodoInicial, int *tamanoRecorrido) {
	...
	// Copiamos el string resultado en un array de caracteres
	char *recorrido = (char*) malloc (sizeof(char)*longitudRecorrido); // Recorrido en forma de array de caracteres (será lo que devuelva la función)
	...
	// Devolvemos el recorrido y su longitud (esto último en el parámetro cuya dirección hemos pasado)
	*tamanoRecorrido = longitudRecorrido;
